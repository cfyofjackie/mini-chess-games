...
>
> 背景：最初遇到 github.io 打不开的情况，曾判断"国内直连不稳定"需要镜像。但 2026-08-30 实测
> （`curl --noproxy` 裸连）github.io 约 1 秒返回 200，甚至快于走代理的 github.com（约 4 秒）——
> 当时的打不开应是代理规则或 DNS 的瞬时问题，并非常态。镜像方案的定位改为"备用 + 提升稳定性"。
> 腾讯云 EdgeOne 国内站因实名认证门槛（需身份证/地址）搁置；备选为复用已有韩国服务器自托管。
