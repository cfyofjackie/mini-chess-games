// 围棋学堂进度本地保存（docs/games/chess.md 第十三节：关卡进度独立存储 go-lessons-completed，
// 与国象学堂的 chess-lessons-completed 互不影响）。localStorage 属浏览器 API，不放引擎目录；
// 解析与去重抽成纯函数，node 环境可单测（读写包装在 Runner 里调用）。
const KEY = 'go-lessons-completed';
